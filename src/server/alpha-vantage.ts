...
export async function fetchAlphaVantageDaily(params: {
  ticker: string;
  apiKey?: string;
}) {
  const apiKey = params.apiKey ?? process.env.ALPHA_VANTAGE_API_KEY;
  ...
  if (!series) {
    throw new Error(typeof note === "string" ? note : "Alpha Vantage response did not include daily time series data.");
  }

  const candles: CandleInput[] = Object.entries(series).map(([date, values]) => ({
    timestamp: fromZonedTime(`${date}T09:30:00`, NY_TIME_ZONE),
    open: Number(values["1. open"]),
    high: Number(values["2. high"]),
    low: Number(values["3. low"]),
    close: Number(values["4. close"]),
